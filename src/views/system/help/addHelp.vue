      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="&nbsp;英文:">
            <div>
              <vue-ueditor-wrap v-model="contentEN"
                                ref="UE_EN"
                                editor-id="editorEN"
                                :config="editorConfig"
                                :editorDependencies="['ueditor.config.js','ueditor.all.js']"
...
import {getCategory, getSetIds, insert} from "@/api/system/help"
...
function onSubmit() {
  ...
  proxy.$refs.helpRef.validate(valid => {
    if(valid){
      insert(tableForm.value).then(response => {
...
getSetIds().then(resres => {
  ...
  setArray.value = resres.map(item => item.SetID)
})
initCategory()
</script>
<style lang="scss">
