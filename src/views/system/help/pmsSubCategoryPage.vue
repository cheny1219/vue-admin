...
import {listCategory, delHelpCategory, categoryAddToUpdate, upOrDownCatagory} from "@/api/system/help"
import { uploadImage } from "@/api/system/upload"
const route = useRoute()
const ParentCategoryID = route.query.hid
...
const data = reactive({
  ...
  queryParams: {
    ...
    updateStartTime: '',
    updateEndTime: '',
    ParentCategoryID : route.query.hid
  },
  rules:{
...
function handleAdd() {
  reset()
  open.value = true
  form.value.ParentCategoryID = ParentCategoryID
}
/** 热门/常见问题按钮操作 */
function toHelpType() {
...
const openTips = (row) =>{
  return new Promise(async (resolve, reject) => {
    if (row.IsShow === true) {
      proxy.$modal.confirms(
          '關閉分類將隱藏所有該分類下的問題<br><span style="color: red;">是否確認關閉？</span>',
          '',
          {
            dangerouslyUseHTMLString: true,
            type: 'warning',
